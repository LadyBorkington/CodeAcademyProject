...
def ngram_creator(text_list):
    ...
    for i in range(len(text_list) - 1):
        word1 = text_list[i]
        word2 = text_list[i + 1]
        two_word_pair = (word1 + "" + word2)
        ngrams.append(two_word_pair)
    return ngrams
...
class TextSample:
    def __init__(self, text, author):
        ...
        average_length = self.average_sentence_length
    ...
    def __repr__(self):
        return "This is {author}'s text and has an average sentence length of {average_length}.".format(author=self.author,average_length=self.average_sentence_length)
...
murder_note_prepared = prepare_text(murder_note)
lily_note_prepared = prepare_text(lily_trebuchet_intro)
myrtle_note_prepared = prepare_text(myrtle_beech_intro)
gregg_note_prepared = prepare_text(gregg_t_fishy_intro)
...
#print (build_frequency_table(murder_note_prepared))
#print (build_frequency_table(lily_note_prepared))
#print (build_frequency_table(myrtle_note_prepared))
#print (build_frequency_table(gregg_note_prepared))
...
def frequency_comparison(table1, table2):
    ...
    return float(mutual_appearances) / float(appearances)
...
def percent_difference(averange_length1, average_length2):
    percent_diff = (abs(averange_length1 - average_length2))/((averange_length1 + average_length2)/2.0)
    return percent_diff
...
def find_text_similarity(text_sample1, text_sample2):
    ...
    end_similarity = ((float(sentence_length_similarity) + float(word_count_similarity) + float(ngram_similarity)) / 3)
...
#lily_score = find_text_similarity(murder_note_prepared, lily_note_prepared)
lily_score = find_text_similarity(murderer_sample, lily_sample)


#myrtle_score = find_text_similarity(murder_note_prepared, myrtle_note_prepared)
myrtle_score = find_text_similarity(murderer_sample, myrtle_sample)

#gregg_score = find_text_similarity(murder_note_prepared, gregg_note_prepared)
gregg_score = find_text_similarity(murderer_sample, gregg_sample)
